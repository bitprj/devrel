...
*Don't get scared by the amount of files to create! 2 of the HTML files are just the starting and ending page, which are very simple, and their corresponding CSS and JavaScript files have a lot in common!*
...
**in JavaScript:**
We want to go to the next page to choose the version when the start button is clicked.
...
**in HTML:**
On this page, we want to have two buttons which redirect to two different HTML pages. The buttons are created in the same way as the start button in the starting page. We add a `<p></p>` to create the text box under each button.

Create two `<div>` with class names "v1" and "v2" for the two buttons under the `<section>` first. These containers will allow us to set the positions of the buttons and the text box later in CSS.
...
**in CSS:**
We can design our button in the corresponding "chooseVer.css" file. To set the style of the entire class, use `.` in front of the class name, or use `#` to refer by the ID name. You could also set styles for everything under a tag by directly putting `p {...}` for the texts for example.
...
This process is similar to the starting page, but now we will have two separate functions corresponding to each of the buttons (invoked by `onclick`) to redirect to different pages.
...
**in JavaScript:**
1. The First Flip:
The first flip is different from the rest, because `word2` is already there and we do not need to reset any word. So we simply use the same `setInterval()` function from our previous timer and call `flipWord()` instead of jumping to the next window. Set the time to 30 seconds.
...
**in JavaScript:**
1. Clicking to Flip:
This is a little different from clicking the button, because we are clicking an element with the tag `<p>`, which does not have an `onclick` feature to add. So we need to add it by ourselves. As Marina Ferreira did in her Memory Game, we use  `addEventlistener()` to tackle with this clicking. 
...
Here we create another function `clicked()` and a Boolean variable `CardIsClicked` to indicate if the card is clicked. This strategy to check the clicked card is also introduced in Marina's Memory Game, but we will be using it differently.
...
Now we are done implementing the clicking version! Time to check out the full game. Refer to the finished code in the [repo](https://github.com/bitprj/Charades_Game_In_JavaScript) if you get a different output.
